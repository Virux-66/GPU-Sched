...
{
    volatile int64_t num_floatingPoint=6511611904;

#if     LEVEL == 3
    volatile int64_t num_transferredBytes=45466;
    volatile float arithmetic_intensity=143220;
#elif   LEVEL == 2
    ...
    cudaMemcpy(d_OptionData,h_OptionData,sizeof(__TOptionData)*optN,cudaMemcpyHostToDevice);


    //checkCudaErrors(cudaMemcpyToSymbol(d_OptionData, h_OptionData, optN * sizeof(__TOptionData)));
    binomialOptionsKernel<<<grid, block>>>(d_OptionData,d_CallValue);
